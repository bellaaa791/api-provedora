...
async function fbdl(videoUrl, cookie, useragent) {
  return new Promise((resolve, reject) => {
    ...
    axios.get(videoUrl, { headers }).then(({ data }) => {
        ...
        if (sdMatch && sdMatch[1]) {
          const result = {
            url: videoUrl,
            duracao_ms: Number(duration[0].split(":")[1]),
            video_sd: parseString(sdMatch[1]),
            video_hd: hdMatch && hdMatch[1] ? parseString(hdMatch[1]) : "",
            titulo: titleMatch && titleMatch[1] ? parseString(titleMatch[1]) : data.match(/<title>(.*?)<\/title>/)?.[1] ?? "",
